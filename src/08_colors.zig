...
pub fn main() !void {
    ...
    const colorVertPath = try join(allocator, &[_][]const u8{ "shaders", "08_colors.vert" });
    const colorFragPath = try join(allocator, &[_][]const u8{ "shaders", "08_colors.frag" });
    ...
    const cubeShader = try Shader.init(allocator, colorVertPath, colorFragPath);
    ...
    while (glfwWindowShouldClose(window) == 0) {
        ...
        const cubeModel = translation(vec3(0.0, 0.0, 0.0));
